export type OsmPlace = { id: string; name: string; address?: string; lat: number; lon: number; distance_m?: number; phone?: string; website?: string; osmUrl?: string };
...
function detect(q:string){ const s=q.toLowerCase(); if (/lawyer|attorney|advocate/.test(s)) return 'lawyer'; for (const k of Object.keys(CATS)) if (s.includes(k)) return k; return null; }
...
export async function overpassPlaces(q:string, lat:number, lon:number, radius=6000): Promise<OsmPlace[]> {
  ...
  const eps = ['https://overpass-api.de/api/interpreter','https://overpass.kumi.systems/api/interpreter','https://overpass.openstreetmap.ru/api/interpreter'];
  for (const ep of eps) {
    try {
      ...
      if (r.ok) {
        ...
        for (const el of j.elements||[]) {
          ...
          out.push({ id:String(el.id), name, address: addr || undefined, lat:c.lat, lon:c.lon, distance_m: Math.round(d), phone:tags.phone||tags['contact:phone'], website:tags.website||tags['contact:website'], osmUrl:`https://www.openstreetmap.org/${el.type}/${el.id}` });
        }
