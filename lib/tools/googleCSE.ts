...
export async function searchCSE(query: string, num = 5): Promise<Hit[]> {
  ...
  u.searchParams.set('q', query); u.searchParams.set('cx', ID); u.searchParams.set('key', KEY); u.searchParams.set('num', String(Math.min(10, Math.max(1, num))));
...
export async function searchCSEMany(queries: string[], perQuery = 3): Promise<Hit[]> {
  ...
  for (const q of queries) {
    ...
    for (const h of part) { if (!seen.has(h.url)) { seen.add(h.url); out.push(h); } if (out.length>=12) break; }
...
export async function findSocialLinks(name: string) {
  ...
  const [wiki, linkedin, insta, fb, x] = await Promise.all([ q('wikipedia.org'), q('linkedin.com'), q('instagram.com'), q('facebook.com'), Promise.race([q('x.com'), q('twitter.com')]) ]);
