...
export default function Home() {
  ...
  function resetAll() {
    setAnswer(''); setCites([]); setCandidates([]); setPlaces([]); setSuggestion(undefined);
    setStatus(undefined); setConfidence(undefined);
  }
    ...
    while (true) {
      ...
      for (const chunk of parts) {
        ...
        try {
          ...
          if (evt.event === 'suggest') setSuggestion(evt.value);
  ...
  return (
    <main className="max-w-3xl mx-auto p-4">
      <div className="flex items-center justify-between mb-4">
        <button onClick={()=>{ setQuery(''); resetAll(); }} className="text-3xl font-bold hover:opacity-80">Wizkid</button>
        <button
      ...
      <form onSubmit={ask} className="flex gap-2 mb-2">
        <input
          ...
          placeholder="Ask anything… e.g., “doctors near me”, “Amitabh Bachchan”, “CLS Foods India Private Limited”"
        />
      ...
      {suggestion && (
        <div className="mb-3 text-sm">
          Did you mean: <button className="underline" onClick={() => { setQuery(suggestion!); ask(undefined, suggestion!); }}>{suggestion}</button>
        </div>
      )}

      {!!candidates.length && (
        <div className="mb-3">
          <div className="text-sm opacity-80 mb-1">Multiple matches — pick one:</div>
      ...
      <article className="prose prose-invert max-w-none bg-white/5 p-4 rounded-2xl min-h-[140px]">
        {status === 'need_location' && <div className="text-xs opacity-80 mb-2">Allow location access to show places near you.</div>}
        {status === 'cse_missing' && <div className="text-xs opacity-80 mb-2">Web search disabled (missing GOOGLE_CSE_KEY / GOOGLE_CSE_ID). Wikipedia fallback used.</div>}
